#!/bin/bash
# Run suite7 tests
# Usage: bash tests/run_suite7.sh build/debug/mycc

CC="${1:-build/debug/mycc}"
TESTDIR="tests/suite7"
PASS=0
FAIL=0
SKIP=0
COMPILE_FAIL=0
...
for src in "$TESTDIR"/test_*.c; do
    ...
    if ! "$CC" -S -o "$asm" "$src" 2>/tmp/${name}_s7_err.txt; then
        printf "  \033[1;31mFAIL\033[0m  %s (compile error)\n" "$name"
        head -2 /tmp/${name}_s7_err.txt | sed 's/^/        /'
        FAIL=$((FAIL + 1))
        COMPILE_FAIL=$((COMPILE_FAIL + 1))
        continue
    fi

    if ! cc "$asm" -o "$bin" 2>/tmp/${name}_s7_err.txt; then
...
echo ""
echo "=== Suite7 Results: $PASS passed, $FAIL failed, $SKIP skipped ==="
if [ $FAIL -gt 0 ]; then
